...
const DTFOrderingSteps = () => {
  ...
  return (
    <section className="bg-blue-50 py-12 md:px-4 ">
      ...
      <p className="text-gray-600 md:text-xl text-center m-10 mb-8">
      ...
      <div className=" gap-8 max-w-5xl mx-auto">
        {steps.map((step, index) => (
          <div
            key={step.id}
            className={`flex md:flex-row flex-col items-center bg-white shadow-lg rounded-lg overflow-hidden ${
              index % 2 === 0 ? "flex-row" : "md:flex-row-reverse"
            }`}
          >
            <img src={step.image} alt={step.title} className="rounded-lg md:w-1/2 object-cover" />
